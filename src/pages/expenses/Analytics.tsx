...
export default function Analytics() {
  ...
  const CHART_COLORS = categoryPalette;
  const STATUS_COLORS = statusPalette;
  const { user } = useAuth();
  ...
  }, [filteredExpenses, STATUS_COLORS]);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <SEO
        title="Analytics"
        description="Detailed breakdown of your expense activity: trends, categories, departments, and merchants."
        path="/analytics"
        noindex
      />
      {/* Header */}
            ...
            {monthlyTrend.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={monthlyTrend} barSize={14}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: axisTickColor }} axisLine={false} tickLine={false} tickFormatter={v => `$${v}`} />
                  <Tooltip
            ...
            {statusBreakdown.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={statusBreakdown} layout="vertical" barSize={14}>
                  ...
                  <YAxis dataKey="status" type="category" width={90} tick={{ fontSize: 10, fill: axisTickColor }} axisLine={false} tickLine={false} />
                  <Tooltip
                    formatter={(val: number, name: string) => name === 'count' ? [val, 'Count'] : [`$${val.toFixed(2)}`, 'Amount']}
                    contentStyle={tooltipContentStyle}
                    itemStyle={tooltipItemStyle}
                    labelStyle={tooltipLabelStyle}
                    cursor={{ fill: gridStroke, opacity: 0.3 }}
