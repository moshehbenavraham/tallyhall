...
export default function MyExpenses() {
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <SEO
        title="My Expenses"
        description="Your submitted, draft, and approved expenses. Filter, search, import, and export."
        path="/expenses"
        noindex
      />
      {/* Header */}
          ...
          {loading ? (
            ...
            <>
              ...
              <Table className="hidden sm:table">
                ...
                <TableBody>
                  {filteredExpenses.map(exp => (
                    // Row-level onClick was a hard window.location.href hop —
                    // breaks SPA routing, isn't keyboard-reachable, and double-fired
                    // with the nested <Link>. The title <Link> below is now the
                    // single accessible click target, with focus-within on the
                    // row so keyboard users see where they are.
                    <TableRow key={exp.id} className="group transition-colors hover:bg-muted/30 focus-within:bg-muted/40">
                      <TableCell>
                        <Link
                          to={`/expenses/${exp.id}`}
                          className="block rounded font-medium transition-colors hover:text-primary hover:underline group-hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                        >
                          {exp.title}
                        </Link>
                      </TableCell>
